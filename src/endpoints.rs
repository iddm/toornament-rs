    ...
    fn fmt(&self, fmt: &mut ::std::fmt::Formatter) -> ::std::fmt::Result {
        ...
        match *self {
            ...
            Endpoint::AllTournaments { with_streams } => {
                address = format!("/v1/tournaments?with_streams={}",
                                  if with_streams { "1" } else { "0" })
            },
            Endpoint::MyTournaments => address = format!("/v1/me/tournaments"),
            Endpoint::TournamentByIdGet { ref tournament_id, with_streams} => {
                address = format!("/v1/tournaments/{}?with_streams={}",
                                  tournament_id.0,
                                  if with_streams { "1" } else { "0" })
            },
            ...
            Endpoint::TournamentCreate => {
                address = format!("/v1/tournaments")
            },
            Endpoint::MatchesByTournament { ref tournament_id, with_games } => {
                address = format!("/v1/tournaments/{}/matches?with_games={}",
                                  tournament_id.0,
                                  if with_games { "1" } else { "0" })
            },
            Endpoint::MatchByIdGet { ref tournament_id, ref match_id, with_games } => {
                address = format!("/v1/tournaments/{}/matches/{}?with_games={}",
                                  tournament_id.0,
                                  match_id.0,
                                  if with_games { "1" } else { "0" })
            },
            Endpoint::MatchByIdUpdate { ref tournament_id, ref match_id } => {
                address = format!("/v1/tournaments/{}/matches/{}",
                                  tournament_id.0,
                                  match_id.0)
            },
            Endpoint::MatchesByDiscipline { ref discipline_id, ref filter } => {
                address = format!("/v1/disciplines/{}/matches?{}",
                                  discipline_id.0,
                                  match_filter(filter.clone()))
            },
            Endpoint::MatchResult(ref tournament_id, ref match_id) => {
                address = format!("/v1/tournaments/{}/matches/{}/result",
                                  tournament_id.0,
                                  match_id.0)
            },
            Endpoint::MatchGames { ref tournament_id, ref match_id, with_stats } => {
                address = format!("/v1/tournaments/{}/matches/{}/games?with_stats={}",
                                  tournament_id.0,
                                  match_id.0,
            ...
            Endpoint::MatchGameByNumberUpdate {
                ...
                address = format!("/v1/tournaments/{}/matches/{}/games/{}",
                                  tournament_id.0,
                                  match_id.0,
                                  game_number.0)
            },
            Endpoint::MatchGameResultGet {
                ...
                address = format!("/v1/tournaments/{}/matches/{}/games/{}/result",
                                  tournament_id.0,
                                  match_id.0,
                                  game_number.0)
            },
            Endpoint::MatchGameResultUpdate {
                ...
                address = format!("/v1/tournaments/{}/matches/{}/games/{}/result?update_match={}",
                                  tournament_id.0,
                                  match_id.0,
                                  game_number.0,
                                  if update_match { "1" } else { "0" })
            },
            Endpoint::Participants { ref tournament_id, ref filter } => {
                address = format!("/v1/tournaments/{}/participants?{}",
                                  tournament_id.0,
            ...
            Endpoint::PermissionById(ref tournament_id, ref permission_id) => {
                address = format!("/v1/tournaments/{}/permissions/{}",
                                  tournament_id.0,
                                  permission_id.0)
            },
            ...
            Endpoint::Videos {
                ...
                address = format!("/v1/tournaments/{}/videos?{}",
                                  tournament_id.0,
...
mod tests {
    ...
    fn test_match_filter_to_get_string() {
        let f = MatchFilter::default().featured(true).has_result(true).page(2i64);
        assert_eq!(match_filter(f), "featured=1&has_result=1&sort=date_asc&with_games=0&page=2");
    }
